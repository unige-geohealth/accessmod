...
# Function to check if shiny-manager is available in the Docker image
_check_shiny_manager() {
    local version="$1"
    local image_tag="$AM5_REPO:$version"
    
    # Try to run shiny-manager -v to check if it's available
    if docker run --rm --entrypoint="" "$image_tag" shiny-manager -v &>/dev/null; then
        return 0  # shiny-manager is available
    else
        return 1  # shiny-manager is not available
    fi
}
...
_start_container() {
    ...
    # Determine the appropriate command based on shiny-manager availability
