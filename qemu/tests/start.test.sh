...
assert_contains "$log" "--health-cmd=wget --spider http://127.0.0.1:3000/health" "modern runtime uses health endpoint"
assert_contains "$log" "-p 3000:3000" "docker run exposes app port"
assert_not_contains "$log" "-p 5000:5000" "modern runtime does not expose legacy http port"
assert_not_contains "$log" "-v /var/run/docker.sock:/var/run/docker.sock" "modern runtime does not mount docker socket"
...
assert_contains "$log" "Rscript --vanilla run.r 3000" "modern runtime passes only the app port"

>"$DOCKER_LOG"
_start_container "5.8.2"
log="$(cat "$DOCKER_LOG")"
assert_contains "$log" "--health-cmd=wget --spider http://127.0.0.1:3000/" "legacy runtime checks the root endpoint"
assert_contains "$log" "-p 5000:5000" "legacy runtime exposes secondary http port"
assert_contains "$log" "-v /var/run/docker.sock:/var/run/docker.sock" "legacy runtime mounts docker socket"
assert_contains "$log" "Rscript --vanilla run.r 3000 5000 8888" "legacy runtime receives all three ports"

assert_success "_uses_legacy_runtime accepts 5.9.0-alpha.3" _uses_legacy_runtime "5.9.0-alpha.3"
assert_success "_uses_legacy_runtime accepts 5.8 prereleases" _uses_legacy_runtime "5.8.3-beta.1"
assert_failure "_uses_legacy_runtime rejects 5.9.0-alpha.4" _uses_legacy_runtime "5.9.0-alpha.4"
assert_failure "_uses_legacy_runtime treats latest as modern" _uses_legacy_runtime "latest"
