...
# structure :
# a.  comment
# b. hint : name of list element. This is used to recognize the error. Be careful with generic text.
# c. type : how to interpret condition : log or error ?
# c. text : replacement text
errMsgList<-list(
  # small warning when removing tmp map used in mask.
  "is a base map for"=list(
    type='log',
    text='base map removed'
    ),
  # v.to.rast warning if convert area is selected, but not found in geometries.
  "No areas selected from vector map"=list(
    type='log',
    text='Vector map conversion to raster without area'
    ),
  # maybe a wrong text. TODO: control this one. The text could be the original message.
  "not recognised by GRASS"=list(
    type='log',
    text='Datum <unknown> not recognised by GRASS'
    ),
  # Error after upload wrong file (type doesn't match extension, no metadata) OR error during gdal operation OR shiny internal fileUpload i/o problem.
  ...
  # occurs  with flag "overwrite" and map exists. 
  ...
  # error during importation if the package raster cant read metadata.
