...
observe({
  ...
  if(!is.null(stackTag) && ! stackTag==''){
    ...
    updateTextInput(session,'stackTag',value=amSubPunct(stackTag,sepTagUi))
  }
})
...
observe({
  ...
  if(!is.null(btnMerge) && btnMerge > 0 && !is.null(sel)){
    amErrorAction(title='Module 1: merge process',{
      ...
      stackTag<-amSubPunct(stackTag,sepTagFile)
      tagSplit<-unlist(strsplit(stackTag,sepTagFile,fixed=T)) #from tag+test+v1#
      merged<-paste(c('merged',paste(tagSplit,collapse='_')),collapse=sepClass)
      bridges<-paste(c('merged_bridge',paste(tagSplit,collapse='_')),collapse=sepClass)
      mapPosition=1
      tempBase<-'tmp__' 
      isFirstMap=TRUE
      rmRastIfExists('tmp_*')
...
landCoverRasterTable<-reactive({
  ...
  if(!is.null(sel)){
    ...
    names(tbl)<-acceptColNames[['table_land_cover']]
...
observe({
  ...
  if(!is.null(sel)){
    ...
    amDebugMsg('Road 2. select road=',sel,', update column sel=',paste(sel,collapse=','),'lab=',paste(lab,collapse=''))
