    ...
    isolate({
      ...
      if(!is.null(btnMerge) && btnMerge > 0 && !is.null(sel)){
        amErrorAction(title='Module 1: merge process',{
          ...
          tagSplit<-unlist(strsplit(stackTag,config$sepTagFile,fixed=T)) #from tag+test+v1#
          merged<-paste(c('merged',paste(tagSplit,collapse='_')),collapse=config$sepClass)
          bridges<-paste(c('merged_bridge',paste(tagSplit,collapse='_')),collapse=config$sepClass)
          mapPosition=1
          tempBase<-'tmp__' 
          isFirstMap=TRUE
          rmRastIfExists('tmp_*')
