  ...
  if(!is.null(btn) && btn>0){
    ...
    amErrorAction(title=paste(input$moduleSelector,' cumulative cost'),{ 
          ...
          if(typeAnalysis == 'anisotropic'){
            ...
            amCreateFrictionMap(tbl,mapMerged,mapFriction)
          }
...
amCreateFrictionMap<-function(tbl,mapMerged,mapFriction){
  ...
  for(i in 1:nrow(tbl)){
    # km/h to s/m * 1000 
    # NOTE: r.reclass output only integer and label. We cant use decimal ! so, time is multiplied by 1000.
    # ---
    # TODO: rewrite this using r.recode for value replacement :
    # oldMin:oldMin:newMin:newMax
    # 23:23:0.35:0.35
    # ---
...
amAnisotropicCapacityAnalysis<-function(inputSpeed,inputFriction,inputPop,inputHf,inputTblHf,outputPopResidual,outputTblHf,outputHfCatchment,removeCapted,typeAnalysis,returnPath,maxCost,radius,groupField,capField,hfOrder=NULL,hfOrderSorting=NULL){

  amDebugMsg('amAnisotropicCapacityAnalysis')

  # nested call if requested order is not given in input hf table
  ...
  # empty df for storing value
  tblOut<-data.frame()
  # set travel time inner ring and outer ring to zero
  amTtInner=0
  amTtOuter=0
  # copy pop map into residual version
  execGRASS('g.copy',raster=c(inputPop,outputPopResidual),flags='overwrite')
  # empty catchment

  execGRASS('r.mapcalc',expression=paste(outputHfCatchment,'=null()'),flags='overwrite')
...
incN=0
  for(i in orderId){
    ...
    # compute anisotrpic cumulated cost map
    ...
    # if NA -> hf capacity is already passed before the first cumulated cost zone. 
    ...
    if(!any(is.na(zInner))&&!length(zInner$zone)==0){
      ...
      execGRASS('r.patch',input=c(outputHfCatchment,tmpPop),output=outputHfCatchment,flags='overwrite')
      # if zero, HF can provide services exactly for the pop within this zone
      #hfCapResidual=hfCap-zInner[nrow(zInner),]$cumSum
      hfCapResidual=hfCap-max(zInner$cumSum)
    }
    # if no inner ring has been comptued, set hfCap as the value to be removed from current or next zone.
    ...
    if(!any(is.na(zOuter)) &&  hfCapResidual>0 && nrow(zOuter)>0){
      ...
      # if whithin outer ring, there isn't enough pop to fill hf capacity, remove maximum  population.
      ...
      execGRASS('r.mapcalc',expression=paste(
          'tmp_pop_residual',"=",outputPopResidual,'-',outputPopResidual,'*',tmpPop,'*',propToRemove
          ),flags="overwrite")
      # patch them with pop residual map
      execGRASS('r.patch',input=c('tmp_pop_residual',outputPopResidual),output=outputPopResidual,flags='overwrite')
      }
    }

    # handle length == 0, for special case :
    # especially when no pop available in cell or in travel time extent
    ...
    names(catDf)<-c(groupField,capField,'amCapacityResidual','amPopWithinMaxDist','amPopInFirstCell','amInnerRingTimeLimit','amOuterRingTimeLimit')
